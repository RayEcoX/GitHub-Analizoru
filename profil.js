document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const [userRes, reposRes] = await Promise.all([
            ...
            fetch(`https://api.github.com/users/${username}/repos?sort=stars&per_page=30&type=owner`)
        ]);
...
function headerSection(u) {
    const location = u.location ? `<span class="me-3"><i class="bi bi-geo-alt me-1"></i>${u.location}</span>` : '';
    const blog     = u.blog     ? `<a href="${u.blog}" target="_blank" style="color:var(--text-secondary);text-decoration:none;"><i class="bi bi-link-45deg me-1"></i>${u.blog}</a>` : '';
    const bio      = u.bio      ? `<p class="mb-3" style="color:var(--text-secondary);line-height:1.7;max-width:600px;">${u.bio}</p>` : '';
    const company  = u.company  ? `<span class="me-3"><i class="bi bi-building me-1"></i>${u.company}</span>` : '';

    return `
        <div class="custom-card p-4 p-md-5 animate-fade-up mb-4">
            <div class="row align-items-center g-4">
                <div class="col-auto">
                    <img src="${u.avatar_url}" alt="${u.login}" class="rounded-circle"
                         style="width:110px;height:110px;object-fit:cover;border:2px solid var(--card-border);">
                </div>
                <div class="col">
                    <h1 class="fw-bolder mb-1" style="color:var(--text-color);font-size:1.8rem;">${u.name || u.login}</h1>
                    <p class="mb-2 fs-5" style="color:var(--text-secondary);">@${u.login}</p>
                    ${bio}
                    <div style="color:var(--text-secondary);font-size:0.88rem;">${company}${location}${blog}</div>
                </div>
                <div class="col-auto">
                    <a href="${u.html_url}" target="_blank" class="btn custom-btn d-flex align-items-center gap-2">
            ...
            <div class="row g-3 text-center">
                <div class="col-6 col-md-3">
                    <div class="stat-box">
                        <div class="fw-bolder fs-3" style="color:var(--text-color);">${formatNum(u.followers)}</div>
                        <div style="color:var(--text-secondary);font-size:0.82rem;">Takipçi</div>
                    </div>
                </div>
                <div class="col-6 col-md-3">
                    <div class="stat-box">
                        <div class="fw-bolder fs-3" style="color:var(--text-color);">${formatNum(u.following)}</div>
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between mb-1">
                    <span style="color:var(--text-color);font-size:0.88rem;font-weight:500;">
                        <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${color};margin-right:6px;"></span>${lang}
                    </span>
...
function insightSection(ins) {
    const rows = [
        ...
        { icon: 'bi-star',             label: 'Toplam Yıldız',        value: formatNum(ins.stars) },
    ].map(r => `
...
function repoSection(repos) {
    ...
    const cards = repos.map(repo => {
        ...
        return `
            <div class="col-md-6 col-lg-4 repo-item" data-lang="${repo.language || 'none'}">
                <a href="${repo.html_url}" target="_blank" class="text-decoration-none">
                    <div class="custom-card p-4 h-100">
                        ...
                        ${repo.language ? `<span class="badge" style="background:${color}20;color:${color};border:1px solid ${color}40;">${repo.language}</span>` : ''}
                    </div>
                </a>
            </div>`;
    }).join('');
...
function notFoundUI(username) {
    return `
        <div class="custom-card p-5 text-center animate-fade-up">
            ...
            <p style="color:var(--text-secondary);"><strong>@${username}</strong> adlı bir GitHub hesabı mevcut değil.</p>
